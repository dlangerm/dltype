...
class Float128Tensor(TensorTypeBase):
    ...
    DTYPES = frozenset((np.float128,)) if is_np_float128_available() else frozenset()
...
class LongDoubleTensor(TensorTypeBase):
    ...
    DTYPES = (
        frozenset((np.longdouble,)) if is_np_longdouble_available() else frozenset()
    )
